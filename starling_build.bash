...
# Define packages to build for Starling
PACKAGES=(
    "coveragecontrol_sim"
    "async_pac_gnn_py"
    "px4_homify"
...
info_message "Starting Starling build for packages: ${PACKAGES[*]}"
...
if colcon build --packages-select "${PACKAGES[@]}" --cmake-args -DCMAKE_BUILD_TYPE=Release; then
    info_message "Starling build completed successfully for all packages"
else
    error_exit "Starling build failed. Check the output above for details."
fi
